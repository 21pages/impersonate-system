...
static int GetTokenInfo(HANDLE TokenHandle)
{
	...
	if (!LookupAccountSid(NULL, pTokenUser->User.Sid, szGroupName, &cchGroupName, szDomainName, &cchDomainName, &SidType)) {
		return -1;
	};
...
static int CreateImpersonatedProcess(HANDLE NewToken, LPCWSTR lpApplicationName)
{
	...
	NewProcess = CreateProcessWithTokenW(NewToken, LOGON_WITH_PROFILE, lpApplicationName, NULL, 0, NULL, NULL, &lpStartupInfo, &lpProcessInformation);
...
extern "C" int run_as_system_c(const char* cmd)
{
	...
	if (StealToken(winLogonPID, lpApplicationName) != 0)
		return -1;
